...
drop procedure if exists email_action;

delimiter //
create Procedure email_action()
begin
 select first_name, last_name, email
  from customer
  join rental on customer.customer_id = rental.customer_id
...
drop procedure if exists email_action2;
...
drop procedure if exists num_movie_cat;

delimiter $$
create procedure num_movie_cat (in param2 int)
begin
...
having count(film.film_id) COLLATE utf8mb4_general_ci > param2;
end $$
delimiter ;

call num_movie_cat(52);
